...
export default function MobilePatrolFlow({ initialScreen = 'selectArea' }) {
  ...
  const handleStartPatrol = async () => {
    ...
    try {
      ...
      if (resp.ok) { const d = await resp.json(); setActiveSession({ ...d, actual_start: startTimeIso, capturedPhotos: [] }); setIsPatrolActive(true); }
    } catch (e) { setActiveSession({ id: 'demo-' + Date.now(), actual_start: startTimeIso, capturedPhotos: [] }); setIsPatrolActive(true); }
  };
